...
def ensure_mineru_api(port: int = DEFAULT_MINERU_PORT, timeout: int = 180) -> bool:
...
def parse_pdf(content: bytes, port: int = DEFAULT_MINERU_PORT) -> dict:
    """提交 PDF 到 MinerU，等待解析完成。

    返回: {total_pages, page_mds: [{page, markdown}], images: {name: bytes}}
    """
    if not ensure_mineru_api(port):
