...
class AnalyticsService:
    ...
    def get_tag_stats(self, db: Session, document_id: Optional[str] = None) -> dict:
    ...
    def report_activity(self, db: Session, seconds: int) -> dict:
        today = _today()
        ...
        return {
            ...
            "total_active_seconds": sum(r.active_seconds or 0 for r in all_rows),
            "today_quiz_seconds": row.quiz_seconds or 0,
    ...
    def get_activity(self, db: Session) -> dict:
        today = _today()
        row = db.query(DailyActivity).filter(DailyActivity.activity_date == today).first()
        all_rows = db.query(DailyActivity).all()
        return {
    ...
    def get_streak(self, db: Session) -> dict:
...
# 模块级单例：调用方仍用 analytics_service.xxx()
