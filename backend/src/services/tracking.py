...
TrackingService 持有追踪领域逻辑；ACHIEVEMENTS 为模块级常量。
"""
...
class TrackingService:
    ...
    def list_reminders(self, db: Session, filter_mode: str = "today") -> dict:
    ...
    def create_reminder(self, db: Session, title: str, remind_date: str) -> Reminder:
    ...
    def update_reminder(self, db: Session, reminder_id: str, title: Optional[str], remind_date: Optional[str], done: Optional[bool]) -> Reminder:
    ...
    def delete_reminder(self, db: Session, reminder_id: str) -> None:
        r = db.get(Reminder, reminder_id)
        if not r:
            raise NotFoundError("提醒不存在")
        db.delete(r)
    ...
    def list_plans(self, db: Session) -> dict:
    ...
    def create_plan(self, db: Session, title: str, goal: Optional[str]) -> StudyPlan:
    ...
    def delete_plan(self, db: Session, plan_id: str) -> None:
    ...
    def list_tasks_by_month(self, db: Session, month: str) -> list[PlanTask]:
        tasks = db.query(PlanTask).filter(PlanTask.due_date.like(f"{month}%")).order_by(PlanTask.due_date).all()
        return tasks

    def create_task(self, db: Session, plan_id: str, title: str, due_date: Optional[str]) -> PlanTask:
    ...
    def update_task(self, db: Session, task_id: str, title: Optional[str] = None, due_date: Optional[str] = None, done: Optional[bool] = None) -> PlanTask:
    ...
    def _progress_map(self, db: Session) -> dict[str, int]:
    ...
    def list_achievements(self, db: Session) -> dict:
        progress_map = self._progress_map(db)
