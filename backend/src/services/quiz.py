...
def get_session(db: Session, session_id: str) -> QuizSession:
...
def get_recent_active_by_document(db: Session, document_id: str) -> Optional[QuizSession]:
    return (
...
def _session_out(db: Session, session: QuizSession) -> dict:
    rows = (
...
async def _grade_by_ai(question: GlobalQuestion, user_answer: Optional[str], qtype: str) -> tuple[str, str]:
...
def update_ref_stats(db: Session, question_id: str, document_id: Optional[str], status: str) -> None:
...
async def submit_answer(
    ...
    # 判题
    status, grade_method, string_match_status, ai_reason = await grade_answer(
        question,
        user_answer,
        status_hint,
    ...
    # 更新统计
    update_ref_stats(db, question_id, session.document_id, status)
...
def get_results(db: Session, session: QuizSession) -> dict:
    rows = (
