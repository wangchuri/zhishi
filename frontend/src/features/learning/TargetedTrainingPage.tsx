...
export function TargetedTrainingPage() {
  ...
  const initTraining = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "启动针对训练失败")
      setPhase("loading")
    }
  }, [])
  ...
  if (phase === "loading" && !error) {
    return (
      <div className="h-full min-h-[calc(100vh-3rem)] flex flex-col items-center justify-center bg-bg gap-4">
        ...
        <p className="text-body text-ink-primary font-medium">AI 正在为你生成专属训练</p>
        <p className="text-small text-ink-tertiary">分析学习报告与薄弱知识点，从题库匹配题目…</p>
      </div>
    )
  }
  ...
  if (error && !session) {
    return (
      <div className="h-full min-h-[calc(100vh-3rem)] flex flex-col items-center justify-center bg-bg gap-4 px-6">
        ...
        <Button variant="primary" onClick={() => { initRef.current = false; initTraining() }}>
          重试
        </Button>
      </div>
    )
  ...
  return (
    <div className="h-full min-h-[calc(100vh-3rem)] flex flex-col bg-bg">
      <header className="shrink-0 border-b border-line-soft bg-surface px-4 py-3 flex items-center justify-between gap-4">
        ...
        <Button variant="ghost" size="sm" onClick={() => { initRef.current = false; initTraining() }}>
          重新生成
        </Button>
      </header>
